...
void mpi_maxlocTree(void *sendbuf, void *recvbuf, int count, MPI_Datatype type) {
	if (type == MPI_INT)
	{
		for (int i = 0; i < count; i++)
		{
...
void mpi_minlocTree(void *sendbuf, void *recvbuf, int count, MPI_Datatype type) {
	if (type == MPI_INT)
	{
		for (int i = 0; i < count; i++)
		{
...
int Tree_reduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype type, MPI_Op op, MPI_Comm comm)
{
		...
		while (j < ProcNum)
		{
			...
			if (k < ProcNum)
			{
				if (ProcRank == j)
				{
					...
					if (op == MPI_MIN) mpi_minTree(sendbuf, recvbuf, count, type);
...
int main(int argc, char *argv[])
{
	...
	//Tree_reduce(mas, mas_res, n, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	...
	//{
	//	cout << endl;
	//	cout << "t =  " << TimeFinish - TimeStart << endl;
	//}
